...
public class ShowManagerDetails extends JFrame {
	...
	private JLabel header,nameLabel,genderLabel,ageLabel,seatsPerRowLabel;
	private JLabel mobileNumberLabel,userNameLabel,addressLabel,passWordLabel;
	private JLabel normalSeatsLabel,balconySeatsLabel,emailLabel;
	private JTextField nameField,mobileNumberField,emailField,userNameField,addressField,ageField;
	private JPasswordField passWordField;
	private JRadioButton maleRadioButton,femaleRadioButton,otherRadioButton;
	private ButtonGroup genderButtonGroup;
	private JTextField normalSeatsField,balconySeatsField,seatsPerRowField;
    ...
    int way;
    int nseats=300,bseats=200,seatsperrow=20;
	public ShowManagerDetails(int way) {
		//settings for window
		this.way = way;
		setResizable(false);
		...
		// creating normal seats field
		normalSeatsLabel = new JLabel("Normal seat rows");
		normalSeatsLabel.setBounds(20, 376, 120, 14);
		panel.add(normalSeatsLabel);
		normalSeatsField = new JTextField();
		normalSeatsField.setBounds(188, 369, 57, 29);
		normalSeatsField.setText(nseats/seatsperrow+"");
		normalSeatsField.setEditable(false);
		
		panel.add(normalSeatsField);		
		
		// creating balcony seats field
		balconySeatsLabel = new JLabel("Balcony seat rows");
		balconySeatsLabel.setBounds(302, 376, 120, 14);
		panel.add(balconySeatsLabel);
		balconySeatsField = new JTextField();
		balconySeatsField.setBounds(470, 369, 57, 29);
		balconySeatsField.setText(bseats/seatsperrow+"");
		balconySeatsField.setEditable(false);
		panel.add(balconySeatsField);
		
		// creating number of seats per row field
		seatsPerRowLabel = new JLabel("Number of Seats in a row ");
		seatsPerRowLabel.setBounds(30, 425, 170, 26);
		contentPane.add(seatsPerRowLabel);
		seatsPerRowField = new JTextField();
		seatsPerRowField.setBounds(196, 425, 58, 26);
		seatsPerRowField.setText(seatsperrow+"");
		seatsPerRowField.setEditable(false);
		contentPane.add(seatsPerRowField);
		
		// adding all fields with existed show manager details
		db.connect();
		ResultSet rs = db.getShowManagerDetails();		
		try {
			if(rs.next()) {
		...
		updateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
					...
					catch(Exception eseats) {
						JOptionPane.showMessageDialog(contentPane, "1)Must be greater than one\n2)Must be number\n3)Should not be empty","Invalid Seats per Row",JOptionPane.ERROR_MESSAGE);												
						return;						
					}
					try {
						nseats =  Integer.parseInt(normalSeatsField.getText());
						if(nseats<1) {
							throw new Exception();
						}
					}
					catch(Exception eseats) {
						JOptionPane.showMessageDialog(contentPane, "1)Must be greater than one\n2)Must be number\n3)Should not be empty","Invalid Normal Seats",JOptionPane.ERROR_MESSAGE);												
						return;						
					}
					try {
						bseats =  Integer.parseInt(balconySeatsField.getText());
						if(bseats<1) {
							throw new Exception();
						}
					}
					catch(Exception eseats) {
						JOptionPane.showMessageDialog(contentPane, "1)Must be greater than one\n2)Must be number\n3)Should not be empty","Invalid Balcony Seats",JOptionPane.ERROR_MESSAGE);												
						return;						
					}					
					nseats*=seatsperrow;
					bseats*=seatsperrow;			
					if(db.isShowManagerExits()) {
						db.updateUser(name,gender,age,mobilenumber,email,address,username,password,id);
					}
					else {
						db.insertUser(name, gender, age, mobilenumber, email, address, username, password, nseats, bseats, seatsperrow, 0);
					}
							
				    JOptionPane.showMessageDialog(contentPane, "The Details have been updated successfully !");										    
				    ShowManagerFrame frame = new ShowManagerFrame(id,name);
	...
	public static void main(String args[]) {
		new ShowManagerDetails(0).setVisible(true);;
	}
}
